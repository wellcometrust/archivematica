...
class TranslationLabel(object):
    ...
    def __init__(self, translations):
        if not isinstance(translations, dict):
            translations = {_DEFAULT_LANG: text_type(translations)}
    ...
    def get_label(self, lang=_DEFAULT_LANG, fallback=None):
        """Get the translation of a message.

        It defaults to English (``_DEFAULT_LANG``) unless ``lang`` is used.
        It accepts a ``fallback``,  used when the message is not available in
        the language given.

        TODO: lang could be a list of accepted languages and the fallback
        mechanism should be able to find general translations, e.g. when
        ``pt_BR`` isn't available we could use ``pt`` instead. See
        ``django.utils.translations.trans_real.get_supported_...``.
        """
        if lang in self._src:
            return self._src[lang]
        if fallback is not None:
            return fallback
        if _DEFAULT_LANG in self._src:
            return self._src[_DEFAULT_LANG]
...
class BaseLink(object):
    ...
    def get_label(self, key, lang=_DEFAULT_LANG, fallback=None):
        """Proxy to find translated attributes."""
        try:
            instance = self._src[key]
        except KeyError:
            return None
        return instance.get_label(lang, fallback)
