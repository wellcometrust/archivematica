...
def test_translation_label(mock):
    ...
    assert tr.get_label(lang="is", fallback="köttur") == "köttur"
    assert tr.get_label(lang="??") == "cat"
    mock.patch("workflow._DEFAULT_LANG", "xx")
    assert tr.get_label(lang="yy") == workflow._UNKNOWN_TRANSLATION_LABEL


def test_translation_label_string():
...
def test_load_valid_document(path):
    ...
    assert first_link.get_label("description") == \
        first_link._src["description"][workflow._DEFAULT_LANG]
