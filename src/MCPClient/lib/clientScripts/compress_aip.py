...
def compress_aip(job, compression, compression_level, sip_directory, sip_name, sip_uuid):
    ...
    _, tool_info, tool_info_err = executeOrRun("bashScript", tool_info_command, printing=True,
                                               capture_output=True)
    ...
    with transaction.atomic():
        for job in jobs:
            with job.JobContext():
                ...
                job.set_status(compress_aip(job,
